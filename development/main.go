...
func main() {
	...
	var ip, httpMethod, path, httpVersion, referrer, userAgent, servicename, logmessage string
	var opts  = strings.Split("userservice,employeeservice,employerservice,authservice", ",")
	var statusCode, bodyBytesSent int
	var timeLocal time.Time

	httpVersion = "HTTP/1.1"
	referrer = "-"
    err := ioutil.WriteFile("/var/www/html/asset/log/services.log", []byte(""), 0644)
    if err != nil {
        log.Fatal(err)
    }
	for range ticker.C {
		...
		servicename = opts[rand.Intn(len(opts))]

		logmessage = fmt.Sprintf("%s - - %s [%s] \"%s %s %s\" %v %v \"%s\" \"%s\"\n", ip,servicename, timeLocal.Format("02/Jan/2006:15:04:05 -0700"), httpMethod, path, httpVersion, statusCode, bodyBytesSent, referrer, userAgent)
		//Append second line
		file, err := os.OpenFile("/var/www/html/asset/log/services.log", os.O_APPEND|os.O_WRONLY, 0644)
		if err != nil {
			log.Println(err)
		}
		defer file.Close()
